...
function generateJwtToken(payload) {
  return jwt.sign(payload, process.env.JWT_SECRET || 'rewardverse_super_secret_jwt_key_2026', {
    expiresIn: '30d'
  });
}
...
export const signupUser = async (req, res) => {
  try {
    ...
    const [insertResult] = await pool.query(
      `INSERT INTO users 
        (uid, user_id, name, email, phone_number, profile_pic, location, referred_by, fcm_token, android_id, referral_code, balance, created_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0.00, NOW())`,
      [
        uid,
        publicUserId,
    ...
    if (android_id) {
      ...
      await pool.query(
        `INSERT INTO device_fingerprints (id, user_id, android_id, device_model, os_version, ip_address, created_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW())
         ON DUPLICATE KEY UPDATE device_model=VALUES(device_model), os_version=VALUES(os_version), ip_address=VALUES(ip_address)`,
        [uuidv4(), insertedId, android_id, device_model || null, os_version || null, ipAddress]
      ).catch(dfErr => console.error('Failed to log device fingerprint:', dfErr));
    }

    // Handle referral mapping if referred_by is set
    if (referred_by) {
      await handleReferralMapping(insertedId, referred_by);
    }

    const [newUserRows] = await pool.query('SELECT * FROM users WHERE id = ? LIMIT 1', [insertedId]);
    const newUser = newUserRows[0];

    res.json({
      success: true,
      message: 'User created successfully',
      token: generateLegacyToken(insertedId),
      jwt: generateJwtToken({ id: insertedId, role: 'user' }),
      user: newUser
    });
  } catch (error) {
...
export const loginAdmin = async (req, res) => {
  try {
    ...
    const adminPass = process.env.ADMIN_PASSWORD || 'admin_rewardverse_secure_pass';
